...
__device__ void Interpolate_FS(int location, int offset, int sign, Argument_Pointers* arg){
	...
	for (int j = 1; j < M; j++){
		if (t_u[location  * width + j] > 0){
			...
			FS[location * width  + j] = FS[i * width + j] + (FS[(i + 1) * width + j] - FS[i * width + j] )* (deltaX / dX - i_deltaX / i_dX);
		}
	}
