...
__device__ DOUBLE source_kd(DOUBLE wsm , DOUBLE Zf, DOUBLE fs, DOUBLE fw, DOUBLE vth, DOUBLE h_moi){
	DOUBLE toee = Toe;
	DOUBLE todd = Tod;
	DOUBLE S = 0;
	DOUBLE Pd, beta, Cb;
...
__device__ DOUBLE source_kkd(DOUBLE wsm, DOUBLE Zf, DOUBLE Dxr, DOUBLE Ufr, DOUBLE Uf, DOUBLE h_moi, DOUBLE fs){
	DOUBLE S = 0;
...
__device__ void _FSi_calculate__mactrix_coeff(bool ketdinh, int i, int j, int first, int last, bool bienran1, bool bienran2, Argument_Pointers* arg, Array_Pointers* arr){

	__shared__ DOUBLE* FS, *tFS, *H_moi, *AA, *BB, *CC, *DD, *t_u, *t_v, *VTH, *Htdu, *Htdv, *Fw, *Kx, *Ky;
	...
	DOUBLE c = 7.8 * log(12 * H_moi[pos] / Ks);
	DOUBLE Uf = sqrt(g) * abs(VTH[pos]) / c;
	DOUBLE wsm = wss() *  pow((1 - FS[pos] / ros), 4);
	DOUBLE Zf = 0;
	...
	DOUBLE S;
	if (ketdinh)
		S = source_kd(wsm, Zf, FS[pos], Fw[pos], VTH[pos], H_moi[pos] );
	else
		S = source_kkd(wsm, Zf, Dxr(), Ufr(), Uf, H_moi[pos], FS[pos]);
	...
	DOUBLE p, q;
	p = q = 0;
	if ((H_moi[(i + 2) * width + j] > H_TINH) &&  (H_moi[(i - 1) * width + j] > H_TINH))
	    p = (FS[(i + 1)  * width + j] - FS[pos]) / dX;

	if  (H_moi[(i - 2) * width + j] > H_TINH && H_moi[(i + 1) * width + j] > H_TINH)
	    q = (FS[pos] - FS[(i - 1) * width + j]) / dX;
	p = (1 / (H_moi[pos] * (dXbp))) * (Htdu[pos] * Kx[pos] * p - Htdu[(i - 1) * width + j] * Kx[(i - 1) * width + j] * q);
	if (H_moi[(i + 2) * width + j] > H_TINH && H_moi[(i - 2) * width + j] > H_TINH)
	    q = (FS[(i + 1) * width + j] - FS[(i - 1) * width + j]) / dX2;
	q = (t_u[pos] + t_u[pos - width]) * 0.5  * q * gamav;
	DD[offset + j] = FS[pos] / (dT * 0.5) - q + p + (S / H_moi[pos]);

	if (j == first + 1){
		if ((!bienran1) && (t_v[i * width + last] < 0) ){
			DD[0] = DD[0] - AA[0] * tFS[i * width + first];
		} else{
			BB[0] = BB[0] + AA[0];
		}
...
__device__ void _FSi_extract_solution(int i, int j, int first, int last, bool bienran1, bool bienran2, Argument_Pointers* arg, Array_Pointers* arr){
	...
	__shared__ DOUBLE* FS, *tFS, *x, *t_v;
	__shared__ int width;
	width = arg->M + 3;
	x = arr->x;
	FS = arg->FS;
	...
	if (j == first + 1){
		...
		if ((!bienran1) && (t_v[i * width + first] < 0))
			FS[i * width + first] = tFS[i * width + first];
	}
	if (j == last - 1){
		FS[i * width + last] = FS[i * width + last - 1];
		if ((!bienran2) && (t_v[i * width + last] < 0))
			FS[i * width + last] = tFS[i * width + last];
	}
}

__device__ void _FSj_calculate__mactrix_coeff(bool ketdinh, int i, int j, int first, int last, bool bienran1, bool bienran2, Argument_Pointers* arg, Array_Pointers* arr){
    ...
    if ((H_moi[pos - 1] > H_TINH) && (H_moi[pos + 2] > H_TINH))
        p = (FS[pos + 1] - FS[pos]) / dY;
    if ((H_moi[pos - 2] > H_TINH) && (H_moi[pos + 1] > H_TINH))
        q = (FS[pos] - FS[pos - 1]) / dY;
    ...
    if (i == first + 1){
	    if ((!bienran1) && (t_u[first * width + j] > 0))
	    	...
	    	DD[0] -= AA[0] * tFS[first * width + j];
	    else
	    	BB[0] += BB[0];
	    if ((!bienran2) && (t_u[last * width + j] < 0)){
	    	DD[SN] -= CC[SN] * tFS[last * width + j];
	    } else
	    	BB[SN] += CC[SN];
	}    
...
__device__ void _FSj_extract_solution(int i, int j, int first, int last, bool bienran1, bool bienran2, Argument_Pointers* arg, Array_Pointers* arr){
	...
	__shared__ DOUBLE* FS, *tFS, *x, *t_u;
	__shared__ int width;
	width = arg->M + 3;
	x = arr->x;
	FS = arg->FS;
	...
	if (i == last - 1){
		if ((!bienran2) && (t_u[last * width + j] < 0)){
	    	FS[last * width + j] = tFS[last * width + j];
	    } else
	    	FS[pos + width] = FS[pos];
	}
}
...
__device__ void _sediment_Transport(DOUBLE t, bool ketdinh, int i, int j, int first, int last, bool bienran1, bool bienran2, Argument_Pointers* arg, Array_Pointers* arr){
	...
	int t1 = (int) (t / 3600);
    if ((t1 % 3) == 0) {
        htaiz[pos] = dT / (1 - Dorong) * (S + tH) + htaiz[pos];
...
// find first, last, bienran1, bienran2, i, j, pass argument
    ...
    _FSi_calculate__mactrix_coeff(ketdinh, i, j, first, last, bienran1, bienran2, arg, arr);
}
...
// find first, last, bienran1, bienran2, i, j, pass argument
    ...
    _FSj_calculate__mactrix_coeff(ketdinh, i, j, first, last, bienran1, bienran2, arg, arr);
}
...
__global__ void Sediment_Transport(DOUBLE t, bool ketdinh, int startidx, int endidx, Argument_Pointers* arg, Array_Pointers* arr){
	int i = blockIdx.y * blockDim.y + threadIdx.y + startidx;
    int j = blockIdx.x * blockDim.x + threadIdx.x + 2;
    if (i >= endidx ) return;
    bool bienran1 = false;
    ...
    int first = 0; int last = 0;
    locate_segment_v(arg->N, arg->M, &bienran1, &bienran2, &first, &last, i, j, arg->daui, arg->cuoii, arg->moci, arg->h);
    _sediment_Transport(t, ketdinh, i, j, first, last, bienran1, bienran2, arg, arr);
}